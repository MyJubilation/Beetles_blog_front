				...
				<div style="flex-grow: 1; max-width: 84%; margin: 8px 0 4px 0;">
					...
					<div class="detailsNaviInfo">
						<div style="display: flex; align-items: center; color: #999; margin-right: 20px; cursor: default;">
							...
							<span style="margin-left: 3px;">阅读 {{ viewCount }}</span>
						</div>
						<a class="detailsNaviInfo-item" @click="giveALike()" href="">
							<el-icon><Pointer /></el-icon>
							<span style="margin-left: 3px;">{{ viewCount }} 赞</span>
						</a>
						<a class="detailsNaviInfo-item" @click="giveAStar()" href="">
							<el-icon><Star /></el-icon>
							<span style="margin-left: 3px;">收藏 {{ viewCount }}</span>
						</a>
					</div>
				</div>
...
<script setup lang="ts">
	...
	const articleList = ref([
		// id, title, summary, author, authorId, avatar, coverImg
	]);
	// const articleList = ref([
	...
	const getDetailsInfoList = async () => {
		...
		try{
			...
			console.log(response.data.listInfo);
